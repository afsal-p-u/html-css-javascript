...
function clickedBox(element){
    if(players.classList.contains('player')){
        element.innerHTML = `<li class="${playerOIcon}"></li>`;
        players.classList.remove('active');
        playerSign = 'O';
        element.setAttribute('id', playerSign);
    }else {
        element.innerHTML = `<li class="${playerXIcon}"></li>`;
...
function bot(){
    ...
    if(array.length > 0){
        if(players.classList.contains('player')){
            allBox[randomBox].innerHTML = `<li class="${playerXIcon}"></li>`;
            players.classList.add('active');
            playerSign = 'X';
            allBox[randomBox].setAttribute('id', playerSign);
        }else {
            allBox[randomBox].innerHTML = `<li class="${playerOIcon}"></li>`;
            players.classList.remove('active');
            allBox[randomBox].setAttribute('id', playerSign);
        }
